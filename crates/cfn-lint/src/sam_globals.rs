    ...
    match (global, local) {
        ...
        (AstNode::Object(g_obj), AstNode::Object(l_obj)) => {
            ...
            let local_keys: std::collections::HashSet<&str> =
                l_obj.entries.iter().map(|e| e.key.as_str()).collect();
            ...
            // Add local entries that weren't in global
            for l_entry in &l_obj.entries {
                if !local_keys.contains(l_entry.key.as_str())
                    || g_obj.entries.iter().any(|e| e.key == l_entry.key)
                {
                    // Skip if already merged above
                    continue;
                }
            }
            // Actually add local-only entries
            for l_entry in &l_obj.entries {
